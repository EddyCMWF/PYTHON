...
if __name__=='__main__':
	...
	infile, maskfile, outfile, miname, maskname, mv = r.parse_input()
...
# Then open the mask file
	...
	coords = [ np.array([eastings[i],northings[i]]) for i in range(npts) ]
	#for i in range(npts):
...
#
	...
	missing=np.where(np.logical_and(indata.mask,maskdata>0))[0]
