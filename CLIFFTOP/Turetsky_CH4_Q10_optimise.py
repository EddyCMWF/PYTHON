...
CH4_depth_factor_NPP = np.exp(-tau_resp*ztot)*dz_soil*tau_resp
...
# Open File
outf=open(OUT_DIR+'fch4_TuretskyQ10_'+time_res+'.txt','w')

Q10_ensemble=np.arange(0.5,15.1,0.1)
nQ10s=len(Q10_ensemble)
GlobTot_ensemble=np.arange(140.,220.1,5.)
...
outf.write( 35*' '+3*'%8s %15s '%('Q10_Cs','k_Cs','Q10_NPP','k_NPP','Q10_Resp_S','k_Resp_s') +'\n' )
Optimised_Q10s={}
...
for gcm in GCMs:
    ...
    NPP    = inf_c.variables['npp_gb'][:].reshape(ntime,nland)
    RESP_S = inf_c.variables['resp_s'][:].reshape(ntime,npools,nz,nland)
    ...
    T_sensitivity_NPP_ensemble= np.zeros([nQ10s,ntime,nland])
    T_sensitivity_RESP_S_ensemble= np.zeros([nQ10s,ntime,nland])
    ...
    for iQ10 in range(nQ10s):
        ...
        for ilayer in range(nz):
            ...
            depth_factor_npp = CH4_depth_factor_NPP[ilayer]
            jules_q10 = JULES_Q10(q10,T_SOIL[:,ilayer,:])
            
            T_sensitivity_NPP_ensemble[iQ10,:]+=NPP*jules_q10*depth_factor_npp
            for ipool in range(npools):
                kappa_weight=Kappa_weights[ipool]
                cs=CS[:,ipool,ilayer,:]
                resp_s=RESP_S[:,ipool,ilayer,:]
                
                T_sensitivity_CS_ensemble[iQ10,:]+=(kappa_weight*cs*jules_q10*depth_factor)
                T_sensitivity_RESP_S_ensemble[iQ10,:]+=(kappa_weight*resp_s*jules_q10)
    ...
    T_sensitivity_RESP_S_correlation = \
            np.array([ stats.pearsonr((T_sensitivity_turetsky*FWETL_AREA).flatten(),
                                      (T_sensitivity_RESP_S_ensemble[iQ10,:]*FWETL_AREA).flatten())[0]
                                             for iQ10 in range(nQ10s) ]).flatten() 

    T_sensitivity_NPP_correlation = \
            np.array([ stats.pearsonr((T_sensitivity_turetsky*FWETL_AREA).flatten(),
                                      (T_sensitivity_NPP_ensemble[iQ10,:]*FWETL_AREA).flatten())[0]
                                                 for iQ10 in range(nQ10s) ]).flatten() 

    CS_FACTORS     = np.zeros([nGlobTots,nQ10s])
    RESP_S_FACTORS = np.zeros([nGlobTots,nQ10s])
    NPP_FACTORS    = np.zeros([nGlobTots,nQ10s])

    RMSE_CS = np.zeros([nGlobTots,nQ10s])
    RMSE_RESP_S = np.zeros([nGlobTots,nQ10s])
    RMSE_NPP = np.zeros([nGlobTots,nQ10s])

    RMSE_CS_scaled = np.zeros([nGlobTots,nQ10s])
    RMSE_RESP_S_scaled = np.zeros([nGlobTots,nQ10s])
    RMSE_NPP_scaled = np.zeros([nGlobTots,nQ10s])
    ...
    for iGlobTot in range(nGlobTots):
        ...
        CH4_RESP_S_temporary  = T_sensitivity_RESP_S_ensemble*FWETL_AREA*(16.04/12.01)
        CH4_NPP_temporary     = T_sensitivity_NPP_ensemble*FWETL_AREA*(16.04/12.01)
        ...
        CH4_RESP_S_temporary  = np.mean(CH4_RESP_S_temporary*secs_to_year,axis=1)
        CH4_NPP_temporary     = np.mean(CH4_NPP_temporary*secs_to_year,axis=1)
        ...
        RESP_S_FACTORS[iGlobTot,:] = (YEAR_tot) / np.sum(CH4_RESP_S_temporary,axis=1)
        NPP_FACTORS[iGlobTot,:]    = (YEAR_tot) / np.sum(CH4_NPP_temporary,axis=1)
        ...
        FCH4_CS     = np.array([T_sensitivity_CS_ensemble[iQ10,:]    *CS_FACTORS[iGlobTot,iQ10]   \
                                ...
                                for iQ10 in range(nQ10s) ] )
        FCH4_RESP_S = np.array([T_sensitivity_RESP_S_ensemble[iQ10,:]*RESP_S_FACTORS[iGlobTot,iQ10]\
                                                *secs_to_year*(16.04/12.01)
                                for iQ10 in range(nQ10s) ] )
        FCH4_NPP    = np.array([T_sensitivity_NPP_ensemble[iQ10,:]   *NPP_FACTORS[iGlobTot,iQ10]   \
                                                *secs_to_year*(16.04/12.01)
                                for iQ10 in range(nQ10s) ] )
        
        RMSE_CS_scaled[iGlobTot,:]     = \
                np.array( [ np.mean( (((FCH4_turetsky_scaled-FCH4_CS[iQ10,:])*FWETL)**2.)**0.5 )
                                              for iQ10 in range(nQ10s) ] )
        RMSE_RESP_S_scaled[iGlobTot,:] = \
                np.array( [ np.mean((((FCH4_turetsky_scaled-FCH4_RESP_S[iQ10,:])*FWETL)**2.)**0.5)
                                              for iQ10 in range(nQ10s) ] )
        RMSE_NPP_scaled[iGlobTot,:]    = \
                np.array( [ np.mean((((FCH4_turetsky_scaled-FCH4_NPP[iQ10,:])*FWETL)**2.)**0.5)
                                              for iQ10 in range(nQ10s) ] )
        RMSE_CS[iGlobTot,:]     = \
                np.array( [ np.mean( (((FCH4_turetsky-FCH4_CS[iQ10,:])*FWETL)**2.)**0.5 )
                                              for iQ10 in range(nQ10s) ] )
        RMSE_RESP_S[iGlobTot,:] = \
                np.array( [ np.mean((((FCH4_turetsky-FCH4_RESP_S[iQ10,:])*FWETL)**2.)**0.5)
                                              for iQ10 in range(nQ10s) ] )
        RMSE_NPP[iGlobTot,:]    = \
                np.array( [ np.mean((((FCH4_turetsky-FCH4_NPP[iQ10,:])*FWETL)**2.)**0.5)
                                              for iQ10 in range(nQ10s) ] )
    ...
    NPP_r_index= np.where(T_sensitivity_NPP_correlation>(min_corre*T_sensitivity_NPP_correlation.max()))[0]
    NPP_Q10_index = NPP_r_index[ np.argmin(RMSE_NPP_scaled[glob_tot_index,NPP_r_index]) ]
    NPP_Q10=Q10_ensemble[NPP_Q10_index]
    NPP_Q10_rval=T_sensitivity_NPP_correlation[NPP_Q10_index] 
    NPP_Q10_RMSE=RMSE_NPP_scaled[glob_tot_index,NPP_Q10_index]
    NPP_kFACTOR=NPP_FACTORS[glob_tot_index,NPP_Q10_index]

    RESP_S_r_index= np.where(T_sensitivity_RESP_S_correlation>(min_corre*T_sensitivity_RESP_S_correlation.max()))[0]
    RESP_S_Q10_index = RESP_S_r_index[ np.argmin(RMSE_RESP_S_scaled[glob_tot_index,RESP_S_r_index]) ]
    RESP_S_Q10=Q10_ensemble[RESP_S_Q10_index]
    RESP_S_Q10_rval=T_sensitivity_RESP_S_correlation[RESP_S_Q10_index] 
    RESP_S_Q10_RMSE=RMSE_RESP_S_scaled[glob_tot_index,RESP_S_Q10_index]
    RESP_S_kFACTOR=RESP_S_FACTORS[glob_tot_index,RESP_S_Q10_index]

    Optimised_Q10s[gcm] = { 
            'CS':    { 'Q10':CS_Q10,'r':CS_Q10_rval,'rmse':CS_Q10_RMSE,'k':CS_kFACTOR},
            'NPP':   { 'Q10':NPP_Q10,'r':NPP_Q10_rval,'rmse':NPP_Q10_RMSE,'k':NPP_kFACTOR},
            'RESP_S':{ 'Q10':RESP_S_Q10,'r':RESP_S_Q10_rval,'rmse':RESP_S_Q10_RMSE,'k':RESP_S_kFACTOR},
                            }

    outf.write( '%8.1f  %15.3e'%tuple( [ Optimised_Q10s[gcm]['CS'][key] for key in ['Q10','k'] ] ) )
    outf.write( '%8.1f  %15.3e'%tuple( [ Optimised_Q10s[gcm]['NPP'][key] for key in ['Q10','k'] ] ) )
    outf.write( '%8.1f  %15.3e'%tuple( [ Optimised_Q10s[gcm]['RESP_S'][key] for key in ['Q10','k'] ] ) )
    outf.write('\n')
    ...
    ax.plot(Q10_ensemble,T_sensitivity_NPP_correlation,label='NPP-r',c='r')
    ax.plot([Optimised_Q10s[gcm]['NPP']['Q10'],Optimised_Q10s[gcm]['NPP']['Q10']],[0,1.0],ls='--',c='r',lw=2)
    ax.plot(Q10_ensemble,T_sensitivity_RESP_S_correlation,label='Resp_S-r',c='g')
    ax.plot([Optimised_Q10s[gcm]['RESP_S']['Q10'],Optimised_Q10s[gcm]['RESP_S']['Q10']],[0,1.0],ls='--',c='g',lw=2)
    ax.plot(Q10_ensemble,T_sensitivity_CS_correlation,label='Cs-r',c='b')
    ax.plot([Optimised_Q10s[gcm]['CS']['Q10'],Optimised_Q10s[gcm]['CS']['Q10']],[0,1.0],ls='--',c='b',lw=2)
    ax.set_ylabel("Pearson's r")
    ax.set_ylim([0,ax.get_ylim()[-1]])
    ax.set_xlabel("Q10")
    ax.set_xlim([Q10_ensemble[0],Q10_ensemble[-1]])
    ax.grid(True)
    ax2=ax.twinx()
    ax2.plot(Q10_ensemble,RMSE_NPP_scaled[glob_tot_index,:],label='NPP-RMSE',ls=':',c='r',lw=2)
    ax2.plot(Q10_ensemble,RMSE_RESP_S_scaled[glob_tot_index,:],label='Resp_S-RMSE',ls=':',c='g',lw=2)
    ax2.plot(Q10_ensemble,RMSE_CS_scaled[glob_tot_index,:],label='Cs-RMSE',ls=':',c='b',lw=2)
    ax2.set_ylabel("RMSE (kgCH4 m$^{-2}$ year$^{-1}$)")
    ax2.set_xlim([Q10_ensemble[0],Q10_ensemble[-1]])
    handles1,labels1 = ax.get_legend_handles_labels()
    handles2,labels2 = ax2.get_legend_handles_labels()
    labels=[labels1[0],labels2[0],labels1[1],labels2[1],labels1[2],labels2[2]]
    handles=[handles1[0],handles2[0],handles1[1],handles2[1],handles1[2],handles2[2]]
    fig.legend(handles,labels, loc=8,ncol=3)
    fig.subplots_adjust(bottom=0.25)
    fig.savefig(OUT_DIR+'plots/'+gcm+'_pearsonsr_rmse180Tg.png')
    plt.close()
    ...
    ax.grid(True)
    ax2=ax.twinx()
    ax2.plot(Q10_ensemble,RMSE_NPP[glob_tot_index,:],label='NPP-RMSE',ls=':',c='r',lw=2)
    ax2.plot(Q10_ensemble,RMSE_RESP_S[glob_tot_index,:],label='Resp_S-RMSE',ls=':',c='g',lw=2)
    ax2.plot(Q10_ensemble,RMSE_CS[glob_tot_index,:],label='Cs-RMSE',ls=':',c='b',lw=2)
    ax2.set_ylabel("RMSE (kgCH4 m$^{-2}$ year$^{-1}$)")
    handles1,labels1 = ax.get_legend_handles_labels()
    handles2,labels2 = ax2.get_legend_handles_labels()
    labels=[labels1[0],labels2[0],labels1[1],labels2[1],labels1[2],labels2[2]]
    ...
    fig.legend(handles,labels, loc=8,ncol=3)
    fig.subplots_adjust(bottom=0.25)
    fig.savefig(OUT_DIR+'plots/'+gcm+'_pearsonsr_rmse180Tg_unscaled.png')
    plt.close()

    
    # Contour plots - Scaled
    fig2,axes=plt.subplots(ncols=3,nrows=1,figsize=(24,10))
    ...
    ax2.plot([Optimised_Q10s[gcm]['CS']['Q10'],Optimised_Q10s[gcm]['CS']['Q10']],[0,0.7],ls=':',c='r',lw=3)
    ax2.set_xlim(Q10_ensemble[0],Q10_ensemble[-1])
    ax2.set_ylim(0,0.5)
    ...
    #Plot NPP
    ax=axes[1]
    im=ax.contourf(Q10_ensemble,GlobTot_ensemble,RMSE_NPP_scaled,cmap='gray')
    ax.set_xlim(Q10_ensemble[0],Q10_ensemble[-1])
    ax.set_ylabel('Global Total (Tg CH4 per year)')
    ax.set_xlabel('Q10 (NPP)')
    ax.set_xlim([Q10_ensemble[0],Q10_ensemble[-1]])
    cax=fig2.colorbar(im,ax=ax,orientation='horizontal')  #,pad=0.15)
    cax.set_label('RMSE (Gg per year)')
    ax2=ax.twinx()
    ax2.plot(Q10_ensemble,T_sensitivity_NPP_correlation,lw=3,c='r')
    ax2.plot([Optimised_Q10s[gcm]['NPP']['Q10'],Optimised_Q10s[gcm]['NPP']['Q10']],[0,1.0],ls=':',c='r',lw=3)
    ax2.set_xlim(Q10_ensemble[0],Q10_ensemble[-1])
    ax2.set_ylim(0,0.5)
    ax2.set_ylabel("Pearson's R")
    ax.set_title("NPP")
    #Plot RESP_S
    ax=axes[2]
    im=ax.contourf(Q10_ensemble,GlobTot_ensemble,RMSE_RESP_S_scaled,cmap='gray')
    ax.set_xlim(Q10_ensemble[0],Q10_ensemble[-1])
    ax.set_ylabel('Global Total (Tg CH4 per year)')
    ax.set_xlabel('Q10 (RESP_S)')
    ax.set_xlim([Q10_ensemble[0],Q10_ensemble[-1]])
    cax=fig2.colorbar(im,ax=ax,orientation='horizontal')  #,pad=0.2)
    cax.set_label('RMSE (Gg per year)')
    ax2=ax.twinx()
    ax2.plot(Q10_ensemble,T_sensitivity_RESP_S_correlation,lw=3,c='r')
    ax2.plot([Optimised_Q10s[gcm]['RESP_S']['Q10'],Optimised_Q10s[gcm]['RESP_S']['Q10']],[0,0.7],ls=':',c='r',lw=3)
    ax2.set_xlim(Q10_ensemble[0],Q10_ensemble[-1])
    ax2.set_ylim(0,0.5)
    ax2.set_ylabel("Pearson's R")
    ax.set_title("Soil Respiration")
    fig2.tight_layout(w_pad=0.3,pad=4)
    fig2.savefig(OUT_DIR+'plots/'+gcm+'_RMSEcontour.png')
    plt.close()

    # Contour plots - Unscaled
    fig2,axes=plt.subplots(ncols=3,nrows=1,figsize=(24,10))
    fig2.subplots_adjust(wspace=0.5)
    #Plot CS
    ax=axes[0]
    im=ax.contourf(Q10_ensemble,GlobTot_ensemble,RMSE_CS,cmap='gray')
    ax.set_ylabel('Global Total (Tg CH4 per year)')
    ax.set_xlabel('Q10 (CS)')
    ax.set_xlim([Q10_ensemble[0],Q10_ensemble[-1]])
    cax=fig2.colorbar(im,ax=ax,orientation='horizontal')  #pad=0.1)
    ...
    #Plot NPP
    ax=axes[1]
    im=ax.contourf(Q10_ensemble,GlobTot_ensemble,RMSE_NPP,cmap='gray')
    ax.set_xlim(Q10_ensemble[0],Q10_ensemble[-1])
    ax.set_ylabel('Global Total (Tg CH4 per year)')
    ax.set_xlabel('Q10 (NPP)')
    ax.set_xlim([Q10_ensemble[0],Q10_ensemble[-1]])
    ...
    ax2=ax.twinx()
    ax2.plot(Q10_ensemble,T_sensitivity_NPP_correlation,lw=3,c='r')
    ax2.plot([Optimised_Q10s[gcm]['NPP']['Q10'],Optimised_Q10s[gcm]['NPP']['Q10']],[0,0.7],ls=':',c='r',lw=3)
    ax2.set_xlim(Q10_ensemble[0],Q10_ensemble[-1])
    ax2.set_ylim(0,0.5)
    ax2.set_ylabel("Pearson's R")
    ax.set_title("NPP")
    #Plot RESP_S
    ax=axes[2]
    im=ax.contourf(Q10_ensemble,GlobTot_ensemble,RMSE_RESP_S,cmap='gray')
    ax.set_xlim(Q10_ensemble[0],Q10_ensemble[-1])
    ax.set_ylabel('Global Total (Tg CH4 per year)')
    ax.set_xlabel('Q10 (RESP_S)')
    ax.set_xlim([Q10_ensemble[0],Q10_ensemble[-1]])
    ...
    ax.set_title("Soil Respiration")
    fig2.tight_layout(w_pad=0.3,pad=4)
    fig2.savefig(OUT_DIR+'plots/'+gcm+'_RMSEcontour_unscaled.png')
    plt.close()
    ...
    if time_res=='Monthly':
        ...
        fig.savefig(OUT_DIR+'plots/'+gcm+'_ZonalTimeSeries.png')
