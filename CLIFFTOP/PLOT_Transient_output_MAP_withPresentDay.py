...
CONFIG= optional_argparse('-config','BASELINE_CONFIG') #_NoLULUC'  #'METHANE_FEEDBACK'  # 
print('Config: ',CONFIG)
DATA_DIR = '/prj/CLIFFTOP/ECP_output/'+CONFIG+'/'
...
OCEAN_DICT = { '1p5deg':np.load(OCEAN_UPTAKE_DIR+'ocean_atmos_gtc_all_1p5_deg_accum.npy')*-1e12,
               '2deg':np.load(OCEAN_UPTAKE_DIR+'ocean_atmos_gtc_all_2_deg_accum.npy')*-1e12,
               }
OCEAN_START_YEAR=1850

print("OCEAN_DICT['1p5deg'].shape=",OCEAN_DICT['1p5deg'].shape)
print("OCEAN_DICT['2deg'].shape=",OCEAN_DICT['2deg'].shape)
...
SCENARIOs = ['presentday','1p5deg'] #, '2deg', '1p81p5deg' ]  # tag in the JULES output file directory 
SCENARIOyears = [PRESENT_DAY_YEAR,2099,2099,2099]  # tag in the JULES output file directory 
                                              #  (see filename construction later)
SCENARIO_names=['Present Day ('+str(PRESENT_DAY_YEAR)+')','1.5$^o$C (2100)','2.0$^o$C (2100)', '1.5$^o$C Overshoot (2100)']  # Name to appear on plots etc.
nSCENARIOs=len(SCENARIOs)
runid='BL'   # runid from jules output (i.e. the start)

# File containing the pre-industrial conditions to use as a baseline:
PI_year=1850
PI_scen='1p5deg'
PI_gcm='CEN_MOHC_MOD_HadGEM2-ES'
#pre_industrial_dump_file=   '/prj/CLIFFTOP/ECP_output/EQUILIBRIUM_OUTPUT/PRE_INDUSTRIAL/' \
...
for iscenario in range(nSCENARIOs):
    ...
    for igcm in range(nGCMs):
        ...
        DUMP_FILE=DATA_DIR+gcm+'/'+runid+'_'+gcm+'_'+scentag+'.dump.'+str(scenyear)+'0101.0.nc'
        print(DUMP_FILE)
        ...
        Ann_File=DATA_DIR+gcm+'/'+runid+'_'+gcm+'_'+scentag+'.Annual_carbon.'+str(scenyear)+'.nc'
        print(Ann_File)
        ...
        OCEAN_CO2 = OCEAN_DICT[scentag][scenyear-OCEAN_START_YEAR,igcm]
