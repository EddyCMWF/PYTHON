...
SC_1ts_file     = 'CRUNCEPn96_simulation_JULESoutput_LAST10_FastSoilCarbon.nc'
...
SC_BS_monthly_file = 'CRUNCEPn96_simulation_JULESoutput_BigSpin_LAST10_monthly_FastSoilCarbon.nc'

n96_landindexfile='/users/eow/edwcom/CRUNCEP/n96/ancil/jules_land_index.nc'

plots_DIR = '/users/eow/edwcom/SC_simulator/output/plots/'
...
inf=nc.Dataset(SCsimulator_DIR+SC_1ts_file,'r')
SC_1ts_C_4pools_Q10t_1D=inf.variables['C_4pools_Q10t'][:]
SC_1ts_C_4pools_RothCt_1D=inf.variables['C_4pools_RothCt'][:]
inf.close()

SC_1ts_C_4pools_Q10t_2D=SC_1ts_C_4pools_Q10t_1D[:,index]
for i in range(4):
    SC_1ts_C_4pools_Q10t_2D[i,:,:][np.where(index.mask==True)]=-999.0

SC_1ts_C_4pools_Q10t_2D=np.ma.masked_equal(SC_1ts_C_4pools_Q10t_2D,-999.)
...
# Open and read in the JULES QuickSpin data
inf=nc.Dataset(JULES_output_DIR+JULES_Quickspin_file)

JULES_QS_SoilCarbon=inf.variables['cs'][:,:,:,:].squeeze()

inf.close()

JULES_QS_SoilCarbon_total=np.sum(JULES_QS_SoilCarbon,axis=1)

JULES_QS_SoilCarbon_mean=np.mean(JULES_QS_SoilCarbon,axis=0)
JULES_QS_SoilCarbon_mean_2D=JULES_QS_SoilCarbon_mean[:,index]
for i in range(4):
    JULES_QS_SoilCarbon_mean_2D[i,:,:][np.where(index.mask==True)]=-999.0

JULES_QS_SoilCarbon_mean_2D=np.ma.masked_equal(JULES_QS_SoilCarbon_mean_2D,-999.)
...
for index,name in zip(GEO_indexes,GEO_names):
    print name, index.shape
    JULES_BS_SoilCarbon_mean_TS=np.mean(JULES_BS_SoilCarbon_total[:,index],axis=1)
    plt.plot(JULES_time,JULES_BS_SoilCarbon_mean_TS,label=name)

plt.legend(loc='upper left')
plt.ylabel('Total Soil Carbon')
plt.show()
