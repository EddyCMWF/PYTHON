...
#print 'SM_sthu.shape =',SM_sthu.shape 
...
nMONTHs     = JULES_source_info['tsteps']   # set at top
nYEARs      = nMONTHs/12.
nPFTs       = LAI.shape[1]
...
out_tag = out_tag+calc_type

#print OUT_DIR+out_tag
...
if (calc_type=='All_timesteps'):
    ...
    # Else if single time step replicate on to all timesteps
        ...
        PFTfrac_work = np.ma.array([PFTfrac for i in range(nMONTHs)])
    ...
    SMwilt_work  = np.mean(SMwilt_sthu.reshape(nYEARs,12,nlandpoints),axis=0)
