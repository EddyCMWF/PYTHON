...
out_tag = 'SCsim_WL_JULES-WFDEI-Zinke-hydro1k'
#
...
# Plot SM diagnostics
if (plot_SMdiags==True):
    plt.subplot(2,2,1)
    plt.imshow(SM[6,grindex-1]*grimask,origin='bottom')
    plt.colorbar()
    plt.subplot(2,2,2)
    plt.imshow(SMsat[grindex-1]*grimask,origin='bottom')
    plt.colorbar()
    plt.subplot(2,2,3)
    plt.imshow(SM_sthf[6,grindex-1]*grimask,origin='bottom')
    plt.colorbar()
    plt.subplot(2,2,4)
    plt.imshow(SMwilt_sthf[grindex-1]*grimask,origin='bottom')
    plt.colorbar()
    plt.show()
...
# 3.1 Average
# PFT params
LAI_climat     = np.mean(LAI.reshape(nYEARs,nMONTHs,nPFTs,nsoilpoints),axis=0)
LAI_mean       = LAI_climat[0,:,:]
LST_climat     = np.mean(LST.reshape(nYEARs,nMONTHs,nPFTs,nsoilpoints),axis=0)
LST_mean       = np.mean(LST,axis=0)
PFTfrac_climat = np.ma.array([PFTfrac for i in range(nMONTHs)])
# Soil Params
SM_sthf_climat = np.mean(SM_sthf.reshape(nYEARs,nMONTHs,nsoilpoints),axis=0)
T_soil_climat  = np.mean(T_soil.reshape(nYEARs,nMONTHs,nsoilpoints),axis=0)
...
if (plot_VEGdiags==True):
    for i in range(nPFTs):
        plt.subplot(2,3,i+1)
        plt.imshow(PFTfrac_climat[i,0,grindex-1]*grimask,origin='bottom',vmin=0,vmax=1)
        plt.colorbar()
        
    plt.subplot(2,3,6)
    plt.imshow(VEGfrac_climat[0,grindex-1]*grimask,origin='bottom',vmin=0,vmax=1)
    plt.colorbar()
    plt.show()
...
LocLit_C_ECP, Cveg_ECP = J_SC_F.JULES_LOCAL_LITTERFALL( LAI_climat, LST_climat,  \
...
if (plot_LITdiags==True):
    for i in range(5):
        plt.subplot(2,3,i+1)
        plt.imshow(Lit_C_ECP[i,grindex-1]*grimask,origin='bottom',vmin=0,vmax=5)
        plt.colorbar()
        
    plt.subplot(2,3,6)
    plt.imshow(Lit_C_total_ECP[grindex-1]*grimask,origin='bottom',vmin=0,vmax=5)
    plt.colorbar()
    plt.show()

if (plot_NFLITdiags):
    plt.subplot(2,2,1)
    plt.imshow((PFTfrac_climat[1,0,grindex-1]*grimask)[280:,540:],\
                 origin='bottom',vmin=0,vmax=1)
    plt.colorbar()

    plt.subplot(2,2,2)
    plt.imshow((LST_climat[1,0,grindex-1]*grimask)[280:,540:],\
                 origin='bottom')###,vmax=233.15) ##,vmax=300)
    plt.colorbar()

    plt.subplot(2,2,3)
    plt.imshow((Lit_C_ECP[1,0,grindex-1]*grimask)[280:,540:], \
                 origin='bottom')
    plt.colorbar()
    
    plt.subplot(2,2,4)
    #plt.imshow((PFTfrac_climat[1,0,grindex-1]*grimask)[280:,540:] * 
    #           (LST_climat[1,5,grindex-1]*grimask)[280:,540:],\
    #             origin='bottom')
    plt.imshow((np.sum(Lit_C_ECP,axis=1)[1,grindex-1]*grimask)[280:,540:], \
                 origin='bottom')    
    plt.colorbar()
...
if (plot_BFLITdiags):
    plt.subplot(2,2,1)
    plt.imshow((PFTfrac_climat[0,0,grindex-1]*grimask),\
                 origin='bottom',vmin=0,vmax=1)
    plt.colorbar()

    plt.subplot(2,2,2)
    plt.imshow((LST_climat[0,0,grindex-1]*grimask),\
                 origin='bottom')###,vmax=233.15) ##,vmax=300)
    plt.colorbar()

    plt.subplot(2,2,3)
    plt.imshow((Lit_C_ECP[0,0,grindex-1]*grimask), \
                 origin='bottom')
    plt.colorbar()
    ...
    #           (LST_climat[1,5,grindex-1]*grimask)[280:,540:],\
    #             origin='bottom')
    plt.imshow((np.sum(Lit_C_ECP,axis=1)[0,grindex-1]*grimask), \
                 origin='bottom')    
    plt.colorbar()
...
plt.colorbar()
plt.subplot(1,2,2)
plt.imshow((Lit_C_ECP[0,6,grindex-1]*grimask), \
             origin='bottom')
plt.colorbar()
plt.show()
...
Carbon_Ones=np.ones([4,nMONTHs,nsoilpoints])
#
...
plt.subplot(1,2,1)
plt.imshow(C_1SC_Q10temp[grindex-1]*grimask,origin='bottom',vmax=100)
plt.colorbar()

plt.subplot(1,2,2)
plt.imshow(C_1SC_RothCtemp[grindex-1]*grimask,origin='bottom',vmax=100)
plt.colorbar()
plt.show()
