...
JULES_output_dir  = '/users/eow/edwcom/SC_simulator/JULES_output/WFDEI_pheno/'
JULES_file        = JULES_output_dir+'JULES_v4.3_WFDEI_RsQ10_GLOBAL_pheno.monthly_mean.nc'
#
# Output Dir
OUT_DIR = '/users/eow/edwcom/SC_simulator/output/'
out_tag = 'SCsim_J4.3_WFDEI_phenoLAI_MarthewsTI_climatology'
#
...
PFTfrac    = inf.variables['frac'][0,:5,:,:].squeeze()
...
SMwilt_sthu= inf.variables['sm_wilt'][0,0,:,:].squeeze() / \
             inf.variables['sm_sat'][0,0,:,:].squeeze()
inf.close()
...
if (plot_VEGdiags==True):
    for i in range(nPFTs):
        plt.subplot(3,2,i+1)
        plt.imshow(PFTfrac_climat[i,0,grindex]*grimask,origin='bottom',vmin=0,vmax=1)
        plt.colorbar()
        
    plt.subplot(3,2,6)
    plt.imshow(VEGfrac_climat[0,grindex]*grimask,origin='bottom',vmin=0,vmax=1)
    plt.colorbar()
    plt.show()

if (plot_LAIdiags==True):
    for i in range(nPFTs):
        plt.subplot(3,2,i+1)
        plt.imshow(LAI_mean[i,grindex]*grimask,origin='bottom')
        plt.colorbar()
...
if (plot_LITdiags==True):
    for i in range(5):
        plt.subplot(3,2,i+1)
        temp_plotdata=np.max(Lit_C_ECP[i,:,:],axis=0)[grindex]*grimask
        plt.imshow(temp_plotdata,origin='bottom',vmin=0,vmax=1)
        plt.colorbar()
        
    plt.subplot(3,2,6)
    temp_plotdata=np.max(Lit_C_total_ECP,axis=0)[grindex]*grimask
    plt.imshow(Lit_C_total_ECP[mnth,grindex]*grimask,origin='bottom',vmin=0,vmax=1)
    plt.colorbar()
    plt.show()
...
#      Calculate Cpool based on Q10 temp function and RothC temp function
C_rpm_Q10t   = (Lit_SCpools_ECP[0,:,:]/Soil_Resp_Fact_Q10t[0,:,:]) / (3600.*24.*360.)
C_rpm_RothCt = (Lit_SCpools_ECP[0,:,:]/Soil_Resp_Fact_RothCt[0,:,:]) / (3600.*24.*360.)
...
if (plot_Cq10diags==True):
    for i in range(4):
        plt.subplot(2,2,i+1)
        plt.imshow(C_4pools_Q10t[i,mnth,grindex]*grimask,\
...
if (plot_Crothdiags==True):
    for i in range(4):
        plt.subplot(2,2,i+1)
        plt.imshow((C_4pools_RothCt[i,mnth,grindex]*grimask), \
                 origin='bottom',vmax=vmaxi[i])
        plt.colorbar()
    plt.show()
...
C_1SC_Q10t   =  (Lit_C_total_ECP/Soil_Resp_Fact_singlepool_Q10t) /  \
                 (3600.*24.*360.)
#C_1SC_Q10temp = np.ma.masked_equal(C_1SC_Q10t,0)
print C_1SC_Q10t.shape

C_1SC_RothCt =  (Lit_C_total_ECP/Soil_Resp_Fact_singlepool_RothCt) /  \
                 (3600.*24.*360.)
