...
INTERVAL='LAST10_monthly_funcmean'
fill_value=-999.
...
#################################################################
# 3.5 Average over time dimension, depending on INTERVAL keyword
##############################
print '"LAST10_monthly" interval selected, averaging over last 10 years for each month'
# Average data over the entire time frame
LAI_JULES=LAI_JULES[:,-120:,:]
LAI_JULES=LAI_JULES.reshape(nPFTs,10,12,nlandpoints)
LAI_JULES=np.mean(LAI_JULES,axis=1)
Tleaf_JULES=Tleaf_JULES[:,-120:,:]
Tleaf_JULES=Tleaf_JULES.reshape(nPFTs,10,12,nlandpoints)
Tleaf_JULES=np.mean(Tleaf_JULES,axis=1)
LAI_BAL_JULES=LAI_BAL_JULES[:,-120:,:]
LAI_BAL_JULES=LAI_BAL_JULES.reshape(nPFTs,10,12,nlandpoints)
LAI_BAL_JULES=np.mean(LAI_BAL_JULES,axis=1)
PFTfrac_JULES=PFTfrac_JULES[:,-120:,:]
PFTfrac_JULES=PFTfrac_JULES.reshape(nPFTs,10,12,nlandpoints)
PFTfrac_JULES=np.mean(PFTfrac_JULES,axis=1)
T_soil_JULES=T_soil_JULES[:,-120:,:]
T_soil_JULES=T_soil_JULES.reshape(nSoilLayers,10,12,nlandpoints)
T_soil_JULES=np.mean(T_soil_JULES,axis=1)
sthu_JULES=sthu_JULES[:,-120:,:]
sthu_JULES=sthu_JULES.reshape(nSoilLayers,10,12,nlandpoints)
sthu_JULES=np.mean(sthu_JULES,axis=1)
sm_wilt_frac_JULES=sm_wilt_frac_JULES[:,-120:,:]
sm_wilt_frac_JULES=sm_wilt_frac_JULES.reshape(nSoilLayers,10,12,nlandpoints)
sm_wilt_frac_JULES=np.mean(sm_wilt_frac_JULES,axis=1)
FracVeg=FracVeg[-120:,:]
FracVeg=FracVeg.reshape(10,12,nlandpoints)
FracVeg=np.mean(FracVeg,axis=0)

#################################################################
# 3.6 index out ice points (i.e. where T_soil=0)
##############################
soil_points=np.where(T_soil_JULES[0,0,:]!=0)[0]
...
C_dpm_Q10t   = np.mean(Lit_SCpools_ECP[0,:,:],axis=0) /  \
               (np.mean(Soil_Resp_Fact_Q10t[0,:],axis=0) *3600.*24.*360.)
C_dpm_RothCt = np.mean(Lit_SCpools_ECP[0,:,:],axis=0) /  \
               (np.mean(Soil_Resp_Fact_RothCt[0,:,:],axis=0) * 3600.*24.*360.)

#
# 6.2 Resistant Plant Material:
#  C_rpm = (f_dpm*Lit_c)/ [Soil_resp_factor]_dpm
#      Calculate Cpool based on Q10 temp function and RothC temp function
C_rpm_Q10t   = np.mean(Lit_SCpools_ECP[0,:,:],axis=0) /  \
               (np.mean(Soil_Resp_Fact_Q10t[1,:],axis=0) * 3600.*24.*360.)
C_rpm_RothCt = np.mean(Lit_SCpools_ECP[1,:,:],axis=0) /  \
               (np.mean(Soil_Resp_Fact_RothCt[1,:,:]) * 3600.*24.*360.)
#
# See Eddy Equations:
# 6.3 Biomass:
...
# Carbon for single pool = total litterfall / soil respiration factor
C_1SC_Q10t   = np.mean(Lit_C_total_ECP,axis=0) / \
               (np.mean(Soil_Resp_Fact_singlepool_Q10t.squeeze(),axis=0)*3600.*24.*365.)
C_1SC_RothCt = np.mean(Lit_C_total_ECP,axis=0) / \
               (np.mean(Soil_Resp_Fact_singlepool_RothCt.squeeze(),axis=0)*3600.*24.*365.)
#
...
C_1SC_Q10t_origgrid=np.zeros([1,lats_orig.shape[0]])+fill_value
C_1SC_RothCt_origgrid=np.zeros([1,lats_orig.shape[0]])+fill_value
# Supplementary data
