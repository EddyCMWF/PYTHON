...
##############################
if (INTERVAL=='ALL'):
    print '"ALL" interval selected, averaging over all time steps'
...
elif (INTERVAL=='LAST10'):
    print '"LAST10" interval selected, averaging over last 10 years'
    # Average data over the entire time frame
    LAI_JULES=np.mean(LAI_JULES[:,-120:,:],axis=1)
    Tleaf_JULES=np.mean(Tleaf_JULES[:,-120:,:],axis=1)
    LAI_BAL_JULES=np.mean(LAI_BAL_JULES[:,-120:,:],axis=1)
    PFTfrac_JULES=np.mean(PFTfrac_JULES[:,-120:,:],axis=1)
    T_soil_JULES=np.mean(T_soil_JULES[:,-120:,:],axis=1)
    sthu_JULES=np.mean(sthu_JULES[:,-120:,:],axis=1)
    sm_wilt_frac_JULES=np.mean(sm_wilt_frac_JULES[:,-120:,:],axis=1)
    FracVeg=np.mean(FracVeg[-120:,:],axis=0)
elif (INTERVAL=='LAST10_monthly'):
...
##############################
if (len(T_soil_JULES.shape)==2):
    soil_points=np.where(T_soil_JULES[0,:]!=0)[0]
