...
inf=nc.Dataset(SCsimulator_DIR+SC_BS_monthly_file,'r')
...
JULES_BS_SoilCarbon_mnth = np.mean(JULES_BS_SoilCarbon[-120:,:,:].reshape(10,12,4,7079),axis=0)
...
for SCpool in range(4):
    ...
    for GEOindex,name in zip(GEO_indexes,GEO_names):
        ...
        plt.plot(np.arange(12)+1,SC_SoilCarbon_mean_TS,label=name)
    #plt.ylim( (0,5) )
    ...
    for GEOindex,name in zip(GEO_indexes,GEO_names):
        ...
        plt.plot(np.arange(12)+1,JULES_BS_SoilCarbon_mean_TS,label=name)
