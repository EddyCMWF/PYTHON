...
#
canradmod=str(input('Select Can Rad Model: '))
...
latlons_BL=zip(lat[BL_index],lon[BL_index])
ulatlons_BL=list(set(latlons_BL))

#site_index=[]
#for i,latlon in zip(range(len(lat)),zip(lat,lon)):
#    if latlon in ulatlons_BL:
#        site_index.append(i)
#
...
plt.plot(x_data,y_data,ls='',marker='.',color='grey')

colours=['blue','red','green','yellow','cyan','darkorange','yellowgreen','purple', \
         'pink','m','darkgreen','royalblue','black','lime','coral','gold']

#colours=[ ,(0.5),(1-(0.05*i) for i in range(len(ulatlons_BL)) ]

col_cnt=0
for site in ulatlons_BL:
    #print site
