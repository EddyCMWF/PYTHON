#!/bin/env python
"""
Class for mapping between WFD-EI global land grid and the Euro sub-domain
used for SWELTER work.  The full grid referred to in this module is a
GRADS-friendly, pole-to-pole grid with the first pixel the south-west corner
and with columns (longitude) varying fastest.
"""
...
class Swmap:
    ...
    def getMap(self, nc=NC_EURO, nr=NR_EURO,
        ...
        the SWELTER Euro2 subsection of the WFD-EI domain."""
    ...
    def getGlobalMap(self,meridians=np.arange(-180,180.1,45),
                     parallels=np.arange( -90, 90.1,15),
                     resolution='l',fontsize=10):
        """Return a regular lon/lat (cylindrical projection) Basemap instance
        covering global WFD-EI domain."""
        XX, YY, M = self.getMap(nc=NC_WFDEI,nr=NR_WFDEI,
                                lon0=LON0_WFDEI,lat0=LAT0_WFDEI,
                                dlon=DLON_WFDEI,dlat=DLAT_WFDEI,
