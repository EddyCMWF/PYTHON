...
outfile_oti='/users/eow/edwcom/WFD_EI/start_dumps/WFDEI_startdump_NG-HWSDSCnofill_20151127.nc'

l_plot_cs_map=True
plotname='/users/eow/edwcom/WFD_EI/SoilCarbonMap_NG-HWSD.png'

grid_file = '/users/eow/edwcom/WFD_EI/EI-Halfdeg-land-elevation.nc'

frac_file = '/users/eow/edwcom/WFD_EI/qrparm.veg.fracNew.nc'
frac_ncname='field1391'
ice_index=8
...
# read in ICE mask
ICEmask = np.where( nc.Dataset(frac_file,'r').variables[frac_ncname][ice_index,:].squeeze() \
                    != 0  )
...
if l_plot_cs_map:
    ...
    PT.plot_map(new_SC,sc_lon_2d,sc_lat_2d,\
                ...
                FILE_PLOT=plotname,
                CBAR_LABEL='$kg$C $m^{-2}$',\
                PLOT_TITLE='Soil Carbon - HWSD (NG)',\
                CBAR_PAD=0.3)
...
#badex  = np.where(new_SC_1D==0)[0]
#goodex = np.where(new_SC_1D!=0)[0]
#print badex.shape
#for pt in badex:
#    temppoint = np.argmin(  ((grlat[pt]-grlat[goodex])**2) \
#              + ((grlon[pt]-grlon[goodex])**2) )
