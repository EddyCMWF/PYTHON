...
if __name__=='__main__':
    ...
    #outfile='/users/eow/edwcom/EMEP/wfd2emep/wfdei_emep4ukEUROPE_startdump_oneSOIL.nc'
    ...
    SurfTypeConvIndex = [2,1,2,1,3,3,5,3,3,5,8,7,9,6]
    ...
    for src_data_name in tile_params:
        ...
        for src_array in src_list:
            ...
            # Replace any bad land points with closest good land point
            if any (remapped_data[dst_mask==1]==-9999.):
                #bad land point index:
                badex  = list(np.where((remapped_data==-9999.)&(dst_mask==1))[0])
                # good land point index:
                goodex = np.where((remapped_data!=-9999.)&(dst_mask==1))[0]
                
                # index of closest good land point to each bad land point
                min_dist_ex = [ np.argmin( \
                                    (dst_cen_lats[badpoint]-dst_cen_lats[goodex])**2 + \
                                    (dst_cen_lons[badpoint]-dst_cen_lons[goodex])**2 ) \
                                for badpoint in badex  ]
                # min_dist_ex is relevant to goodex, hence:
                swap_index=goodex[min_dist_ex]
                # swap data
                remapped_data[badex]=remapped_data[swap_index]
    ...
    outf.note='Surface type conversions from original JULES index = [2,1,2,1,3,3,5,3,3,5,8,7,9,6]'
    outf.close()
