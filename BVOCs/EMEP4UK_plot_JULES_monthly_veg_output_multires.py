...
class plotsoilmaps:
    def parse_input(self):
        ...
        parser.add_argument('--infileUK',type=str,help='Input file UK',required=False, \
                             default='/users/eow/edwcom/EMEP/EMEP4UK/JULES_output/JULES_EMEP4UK_MPI_DougDiag.monthly_veg.nc')
        parser.add_argument('--infileEU',type=str,help='Input file EU',required=False, \
                             default='/users/eow/edwcom/EMEP/EMEP4UK/JULES_output/JULES_EMEP4UK_EUROPE_MPI_DougDiag.monthly_veg.nc')
...
if __name__=='__main__':
    ...
    #
    infileUK = '/users/eow/edwcom/EMEP/EMEP4UK/JULES_output/JULES_EMEP4UK_MPI_DougDiag.monthly_veg.nc'
    infileEU = '/users/eow/edwcom/EMEP/EMEP4UK/JULES_output/JULES_EMEP4UK_EUROPE_MPI_DougDiag.monthly_veg.nc'
    ...
    #
    varnames=['isoprene']  #,'lai','terpene','gpp','c_veg']
    pft_index=np.array([1,3])
    #
    NLEVELSs   = [ 11 for counter in range(len(varnames))]
    cbars      = ['YlGn' for counter in range(len(varnames))]
    ...
    for var in varnames:
        ...
        #
        tempdata=inf1.variables[var][:].squeeze()   # Remove redundant spatial dimensioni
        tempdata=np.amax(tempdata[:,pft_index,:],axis=1)
        ...
        data1[var]['drange']   = [ 0,round2SignifFigs(np.amax(data1[var]['map_max']),2) ]
    #    
    inf1.close()
    #
    # EUROPE data
    ...
    for var in varnames:
        ...
        data2[var]['drange']   = [ 0,round2SignifFigs(np.amax(data2[var]['map_max']),2) ] 
    #    
    inf2.close()
    #
    latrange = [41.,38.]
